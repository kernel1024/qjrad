...
#include "goldendict/dsl.hh"
#include "goldendict/stardict.hh"
#include "goldendict/dictdfiles.hh"
#include "goldendict/wstring_qt.hh"
#include "goldendict/htmlescape.hh"
...
CGoldenDictMgr::CGoldenDictMgr(QWidget *parent) :
    QObject(parent), mainWnd(parent)
{
}
...
void CGoldenDictMgr::loadDictionaries()
{
    ...
    CLoading ldlg(mainWnd);
    ldlg.setWindowModality(Qt::ApplicationModal);
    ldlg.show();
    ldlg.addLogMsg(tr("Indexing dictionaries. Please wait."));

    CDictLoader loadDicts( this, cgl->getDictPaths() );

    QObject::connect( &loadDicts, SIGNAL( indexingDictionarySignal( QString const & ) ),
                      &ldlg, SLOT( addLogMsg( QString const & ) ) );
    ...
    {
      QMessageBox::critical( mainWnd, tr("Error loading dictionaries"),
...
void CDictLoader::handlePath(const QString &path, bool recursive)
{
...

    {
      std::vector< sptr< Dictionary::Class > > stardictDictionaries =
        Stardict::makeDictionaries( allFiles, cgl->getIndexDir().toLocal8Bit().data(), *this );
...

    {
      std::vector< sptr< Dictionary::Class > > dslDictionaries =
        Dsl::makeDictionaries( allFiles, cgl->getIndexDir().toLocal8Bit().data(), *this );
...

    {
      std::vector< sptr< Dictionary::Class > > dictdDictionaries =
        DictdFiles::makeDictionaries( allFiles, cgl->getIndexDir().toLocal8Bit().data(), *this );
